...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    throw new Error('a cannot be zero in a linear equation.');
  }
  return -b / a;
}
...
function getLastDigit(value) {
  ...
  return parseInt(numberString.charAt(numberString.length - 1), 10);
}
...
function parseNumberFromString(value) {
  const cleanedValue = value.match(/-?\d+(\.\d+)?/);
...
function getParallelepipedDiagonal(a, b, c) {
  if (typeof a !== 'number' || typeof b !== 'number' || typeof c !== 'number' || a < 0 || b < 0 || c < 0) {
    throw new Error('Invalid input: a, b, and c must be non-negative numbers.');
  }
